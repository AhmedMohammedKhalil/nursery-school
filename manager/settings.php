...
?>
	<div class="section" id="login" style="margin-bottom: 33px;">
      <div class="container">
        ...
        <form action="<?php echo $cont."ManagerController.php?method=editProfile"?>" method="POST" class="form">
          <?php
            if(isset($_SESSION['errors'])) {
          ...
          <div style="display: flex;justify-content:center;flex-direction:column">
            <div>
                ...
                value="<?php if(isset($_SESSION['errors'])){ echo $username; } else { echo $_SESSION['manager']['username'];}?>">
            </div>
            <div>
                <label for="name">Name</label>
                <input type="text" name="name" id="name" title="Enter name" placeholder="Enter name" required
                value="<?php if(isset($_SESSION['errors'])) { echo $name ;} else { echo $_SESSION['manager']['name'];}?>">
            </div>
            <div>
                <label for="position">Position</label>
                <input type="text" name="position" id="position" title="Enter position" placeholder="Enter position" required
                value="<?php if(isset($_SESSION['errors'])) { echo $position ; } else { echo $_SESSION['manager']['position'];}?>">
            </div>
            <div>
